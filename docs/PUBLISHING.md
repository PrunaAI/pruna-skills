...
There is **no single API** that publishes to all channels. `./.maintainer/release/publish_all_skills.py` automates both ClawHub surfaces; GitHub tag is the source-of-truth release for `npx skills`.
...
A tag alone is not enough for the release page — **always** run `./.maintainer/release/create_github_release.sh <VERSION>` (or let CI create it on `skills-v*` push). Consumers and maintainers use the [Releases](https://github.com/PrunaAI/pruna-skills/releases) page for notes.
...
# 3. Upload to ClawHub
./.maintainer/release/publish_all_skills.sh --execute
...
```bash
./.maintainer/release/publish_all_skills.sh --execute --target clawhub --skill p-image
./.maintainer/release/publish_all_skills.sh --execute --target clawhub-plugins --plugin avatar-multi-scene
```
...
| **ClawHub / OpenClaw** | Yes | `./.maintainer/release/publish_all_skills.sh --execute` (or CI on tag) | `clawhub install @PrunaAI/p-image` |
...
Note: the reusable workflow expects flat skill folders; use `./.maintainer/release/publish_all_skills.py` for nested `plugins/<name>/skills/<name>` layout.
...
```bash
./.maintainer/release/publish_all_skills.sh --execute --target clawhub --skill p-video-replace
...
```bash
./.maintainer/release/publish_clawhub_batches.sh
# background: nohup ./.maintainer/release/publish_clawhub_batches.sh >>output/clawhub-publish-batch.log 2>&1 &
```
