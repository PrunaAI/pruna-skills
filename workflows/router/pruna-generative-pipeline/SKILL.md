...
This skill is the **menu of proven chains** over the atomic tools in `tools/image` and `tools/video`. For HTTP details see [pruna-api.md](../../../../references/shared/pruna-api.md).
...
| **Scenes** | If multiple clips: scene table with distinct backgrounds/angles (or defer to [narrated-multi-scene](../core/narrated-multi-scene/SKILL.md) / [avatar-multi-scene](../core/avatar-multi-scene/SKILL.md))? |
...
Deep avatar workflows already spell out cast ledgers, hero reuse, and read-throughs in [avatar-multi-scene](../core/avatar-multi-scene/SKILL.md) and [avatar-single-scene](../core/avatar-single-scene/SKILL.md); defer to those when recipe **G** or **H** involves **`p-video-avatar`**.
...
| D — **Animate a plate** | One motion clip from a still | `p-image` or upload → `p-video` (I2V) | [image-to-video](../core/image-to-video/SKILL.md) |
| E — **Audio-led cut** | Video length follows VO/music | upload `audio` → `p-video` | [image-to-video](../core/image-to-video/SKILL.md) |
| F — **Draft → final video** | Cheap preview then hi-fi | `p-video` (`draft: true`) then rerun `draft: false` with locked `seed`/prompt | [narrated-multi-scene](../core/narrated-multi-scene/SKILL.md) for many beats |
| G — **Talking head** | Portrait + speech | `p-image` / `p-image-edit` → `p-video-avatar` | [avatar-single-scene](../core/avatar-single-scene/SKILL.md) or [avatar-multi-scene](../core/avatar-multi-scene/SKILL.md) |
...
| M — **Motion-transfer showcase** | Same motion, new subject + slider before/after | motion `.mp4` + still → `p-video-animate` → slider compare MP4 | [avatar-multi-scene](../core/avatar-multi-scene/SKILL.md) (`animate` rows) |
| N — **In-video replacement showcase** | Swap people/products in footage + slider before/after | dynamic `p-image` refs → optional `p-video-avatar` source → `p-video-replace` → slider compare MP4 | [p-video-replace](../../../../tools/video/p-video-replace/SKILL.md) · [avatar-multi-scene](../core/avatar-multi-scene/SKILL.md) |
| O — **AI music video** | Full song + lyric-synced video | lyrics → [music-2.5](../../../../tools/audio/music-2.5/SKILL.md) → cut map → `p-video-avatar` + `p-video` → assembly; hero + `p-image-edit` when one singer throughout | [music-video](../verticals/music-video/SKILL.md) |
| P — **Narrated story film** | Multi-scene B-roll + VO (+ optional bed) | hero → `p-image-edit` start/end stills → `p-video` with `image` + `last_frame_image` chain → [gemini-3.1-flash-tts](../../../../tools/audio/gemini-3.1-flash-tts/SKILL.md) → concat + mux ± [stable-audio-2.5](../../../../tools/audio/stable-audio-2.5/SKILL.md) | [narrated-multi-scene](../core/narrated-multi-scene/SKILL.md) · [audio-post-production.md](../../../../references/audio/audio-post-production.md) |
| Q — **Visual transition reel** | Multi-scene motion between two stills per beat (no VO) | `p-image` hero → `p-image-edit` start/end stills → `p-video` pair (`duration`) → selective frame chain → concat | [visual-transition-reel](../core/visual-transition-reel/SKILL.md) · [scene-anchor-pair.md](../../../../references/video/scene-anchor-pair.md) |
...
| T — **Virtual try-on launch** | Fashion vertical showcase reel | `p-image` person + garment → **`p-image-try-on`** → `p-video-avatar` / `p-video` / still slider → Gemini TTS → concat + **Stable Audio** bed | [p-image-try-on](../../../../tools/image/p-image-try-on/SKILL.md) · [recipe-catalog.md](../../../../references/workflows/recipe-catalog.md) (Recipe T) |
...
- **Photoreal stills:** Hero via **`p-image`** with documentary/photoreal prompt → slop gate; every **`p-image-edit`** still → slop gate before **`p-video-avatar`**. Persona bar: [realistic-persona-showcase.md](../../../../references/shared/realistic-persona-showcase.md).
