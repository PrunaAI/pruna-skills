...
if [[ -z "${SKILL_SRC}" || ! -f "${SKILL_SRC}/SKILL.md" ]]; then
  if [[ "${USE_MINE}" -eq 1 ]]; then
    ...
    echo "Skill not found: ${SKILL} (searched workflows/, tools/)" >&2
...
repo = Path("${REPO_ROOT}")
dest = Path("${DEST}")
refs_roots = [repo / "references" / "policies", repo / "references"]
if ${USE_MINE}:
...
python3 "${REPO_ROOT}/.maintainer/inject_policies.py" "${DEST}"
