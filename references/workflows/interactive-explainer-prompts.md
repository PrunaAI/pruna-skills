...
Extracted reference for [educational-explainer](../../guides/workflows/verticals/interactive-explainer/SKILL.md). The runner enforces these via `validate_plan` and `STILL_PROMPT_TRIGGERS` in [`run_from_plan.py`](../../guides/workflows/verticals/interactive-explainer/scripts/run_from_plan.py).
...
6. **`p-video-avatar`:** `defaults.avatar_negative_prompt` is a **Pruna suppression token list** (nouns like `subtitles`, `watermark`) — not creative wording. See [`p_video_avatar_payload.py`](../../guides/workflows/_shared/scripts/p_video_avatar_payload.py).
...
Broader blocked-phrase table (keyboards, mirrors, packshots): [p-video-replace-comparison/SKILL.md](../../guides/workflows/launches/p-video-replace-comparison/SKILL.md) — link [replace-beats.md](../../guides/workflows/launches/p-video-replace-comparison/replace-beats.md) **Prompt trigger words**.
...
- Workflow skill: [interactive-explainer/SKILL.md](../../guides/workflows/verticals/interactive-explainer/SKILL.md)
