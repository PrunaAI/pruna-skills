...
**Visual-only transitions (no VO):** use the [scene anchor pair](./scene-anchor-pair.md) — `duration` instead of `audio`; see [scene-transition-video](../workflows/core/visual-transition-reel/SKILL.md).
...
**Over 20s?** Shorten scene copy → tighten TTS pace in `style_prompt` → split into two scene rows (each with its own triple). See [multi-scene-ai-video](../workflows/core/narrated-multi-scene/SKILL.md) duration gate.
...
| **Music video** | Music 2.5 full song | — | [ai-music-video](../workflows/verticals/music-video/SKILL.md) |
...
- [multi-scene-ai-video](../workflows/core/narrated-multi-scene/SKILL.md)
