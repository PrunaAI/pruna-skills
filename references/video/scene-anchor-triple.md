...
**20-second ceiling:** audio-led clips cannot run longer than P-API `duration` max (**20s**). Write per-scene TTS to **≤ ~19s** (probe with `ffprobe` after Gemini). Truncated VO with “audio passed” usually means the line was too long, not that `input.audio` was missing. Helper: [`validate_narration_duration`](../guides/workflows/_shared/scripts/p_video_payload.py).
...
- [scene-transition-video](../guides/workflows/core/visual-transition-reel/SKILL.md) — visual pair (no VO)
- [multi-scene-ai-video](../guides/workflows/core/narrated-multi-scene/SKILL.md) — primary narrated workflow
- [single-scene-ai-video](../guides/workflows/core/image-to-video/SKILL.md) — one beat
- [pruna-generative-pipeline](../guides/workflows/router/pruna-generative-pipeline/SKILL.md) — Recipe **P**
