...
**20-second ceiling:** audio-led clips cannot run longer than P-API `duration` max (**20s**). Write TTS to **≤ ~19s** (probe with `ffprobe` after Gemini). Truncated VO with “audio passed” usually means the line was too long, not that `input.audio` was missing. Helper: [`validate_narration_duration`](../workflows/_shared/scripts/p_video_payload.py).
...
1. [Gemini 3.1 Flash TTS](../tools/audio/gemini-3.1-flash-tts/SKILL.md) (or [Music 2.5](../tools/audio/music-2.5/SKILL.md) slice for music videos)
2. Download MP3/WAV
...
| **`p-video-animate`** | `image` + **`video`** (motion template) — different axis; not narration triple — use the [p-video-animate](../tools/video/p-video-animate/SKILL.md) skill |
