...
| `p-video` | Text / image / audio video; **first frame** (`image`) + **last frame** (`last_frame_image`) chaining | `tools/video/p-video` | `references/p-video-quality-checklist.md` |
...
| `gemini-3.1-flash-tts` | Narration / voiceover TTS | `tools/audio/gemini-3.1-flash-tts` | Requires `REPLICATE_API_TOKEN`; mux or drive `p-video` via uploaded audio — [audio-post-production.md](./audio-post-production.md) |
...
| Multi-scene cinematic (`p-video` per scene, scene anchor triple) | `guides/workflows/multi-scene-ai-video` |
