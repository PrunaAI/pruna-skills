...
Shared guidance for **eye-catching** Pruna showcase reels — comparison sliders, announcement plans, and multi-scene pieces.
...
**Never use prompt trigger words** that cause stray text or multi-panel collages — see [p-video-replace-comparison/SKILL.md](../guides/workflows/p-video-replace-comparison/SKILL.md) trigger tables. Prefer positive single-frame wording only.
