...
Execute phases with parallel curl batches — **never** batch `p-video-2` before still and TTS review. Default clip model: `p-video-2` (`p-video` only if the user named the original).
