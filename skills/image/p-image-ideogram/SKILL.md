...
**Route by complexity:** Use `` `p-image-ideogram` `` when **photo generation** needs more control — readable text in the image, structured JSON, hex/`bbox` placement, or high-detail photoreal shots. Use `` `p-image` `` for **simple, quick** photo generation. Use `` `p-image-edit` `` to change an existing photo; use `` `p-video-2-pro` `` / `` `p-video-2` `` (`` `p-video-animate` `` from a still) for motion.
