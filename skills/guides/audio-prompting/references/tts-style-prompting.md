...
Director-style `prompt` craft for `gemini-3.1-flash-tts`. Upload results to Pruna for [audio-in-video-prompting.md](../video/audio-in-video-prompting.md) Mode B. Layering: [audio-post-production.md](./audio-post-production.md).
...
**Bracket clarity:** `[tags]` live only in this TTS `text` field. Still typography uses double-quoted `"[STRING]"` → [prompt-golden-rules.md](../image/prompt-golden-rules.md). Native clip dialogue (`[subject] says "[LINE]"`) → [audio-in-video-prompting.md](../video/audio-in-video-prompting.md) Mode A — not this skill.
