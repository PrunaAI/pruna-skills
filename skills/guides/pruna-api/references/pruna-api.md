...
- `POST /v1/predictions`
- Headers: `Content-Type: application/json`, `apikey`, **`Model: <model-id>`** (for example `p-image-ideogram`, `p-image`, `p-image-edit`, `p-image-try-on`, `p-video-2-pro`, `p-video-2`, `p-video`, `p-video-avatar`, `p-video-animate`, `p-video-replace`, `p-image-upscale`)
...
Typical multi-scene avatar phases: plan → hero `p-image-ideogram` → parallel scene `p-image-edit` → parallel `p-video-avatar` → assembly. Narrated films: parallel Gemini TTS per scene after scripts approved, then parallel `p-video-2` (quality) or `p-video` (simpler) when all anchor triple URLs exist.
