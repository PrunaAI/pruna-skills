...
Output: audio file URL. Use for narration — upload to Pruna as part of scene anchor triple (`video-prompting`) (`input.audio` + `input.image` + `input.last_frame_image` on `p-video-2` or `p-video`). Layering with beds: `audio-prompting`
