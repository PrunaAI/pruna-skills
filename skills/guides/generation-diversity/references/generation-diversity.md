...
- **`text_spec` (stills):** `"[exact string]"` + `[surface]` + `[placement]` → [prompt-golden-rules.md](../image/prompt-golden-rules.md) §5
- **Native clip dialogue:** `p-video` Mode A only (`[subject] says "[LINE]"` + mouth + gesture) → [audio-in-video-prompting.md](../video/audio-in-video-prompting.md) — not `p-image`
- **`[tags]`:** Gemini TTS `text` performance only — not still typography, not `p-video` motion prompt → [tts-style-prompting.md](../audio/tts-style-prompting.md)

**Collage triggers (all T2I models):** still avoid `flat lay`, `packshot`, `grid`, `collage`, `montage`, `contact sheet`, `split`, `before and after` — use `single frame`, `one camera angle` instead. Full table: [interactive-explainer-prompts.md](../../skills/workflows/interactive-explainer/references/interactive-explainer-prompts.md).
...
Full style/camera/lighting ladders: [Visual variety](#visual-variety) below. Persona + try-on bar: [realistic-persona-showcase.md](../shared/realistic-persona-showcase.md).
...
| Rule | Guidance |
|------|----------|
| **Unified bar** | this document · [realistic-persona-showcase.md](../shared/realistic-persona-showcase.md)
...
| [pruna-generative-pipeline](../../docs/WORKFLOW-RECIPES.md) | Intake must capture variety plan before recipe execution |
...
- [realistic-persona-showcase.md](../shared/realistic-persona-showcase.md) — persona planning
