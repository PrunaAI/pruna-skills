...
Follow [avatar-single-scene](../avatar-single-scene/SKILL.md) or [avatar-multi-scene](https://github.com/PrunaAI/pruna-skills/tree/main/plugins/avatar-multi-scene/skills/avatar-multi-scene/SKILL.md): **[generation diversity](./references/generation-diversity.md)** first, then **natural human `voice_script`**, **realistic conversational `voice_prompt`**, **per-scene dynamic `video_prompt`**, **locked hero plate URL**, **one fixed `voice` per recurring character**, **confirm `voice_language` with the user** (examples that use `English (US)` are illustrative only), **explicit user confirmation** before any **`POST /v1/predictions`**, then emit and run the agreed generation steps.
