...
from pruna_api import api_seed_from_plan, create_prediction, download_file, require_api_key, upload_file  # noqa: E402
...
def validate_plan(plan: dict, scenes: list[dict]) -> None:
    if not any(is_character_scene(s) for s in scenes):
        print("Warning: no character scenes — plan is narrator-only; see interactive-explainer skill for interaction mix.")
        ...
        for trig in STILL_PROMPT_TRIGGERS:
            if trig in lower:
                raise RuntimeError(
                    ...
                    f"wording; see visual-variety-bible.md#prompt-patterns"
                )
