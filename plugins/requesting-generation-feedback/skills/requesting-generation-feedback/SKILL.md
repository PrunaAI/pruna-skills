...
Canonical phase rules: [staged-generation-gate.md](./references/staged-generation-gate.md)  
Per-workflow runners and flags: [workflow-feedback-gates.md](./references/workflow-feedback-gates.md)
...
| Regen prompts without deleting stills/clips | Old assets reused | Delete targets or `--fresh` / `--regen-*` per [staged-generation-gate.md](./references/staged-generation-gate.md) partial regen table |
| `voice_script` revised but avatar sources not deleted | Lip sync / dialogue mismatch | Delete `sources/` + `clips/` for that scene |
| **`POST /v1/predictions` without [random seed ritual](https://github.com/PrunaAI/pruna-skills/tree/main/references/shared/random-seed-ritual.md) (SSoT)** | Duplicate outputs; copied example strings | Generate and state a ritual string first; log `ritual_seed` |
| **`PRUNA_API_KEY` or `REPLICATE_API_TOKEN` missing** | Cannot run API or runners | Stop; send signup links from [api-credentials.md](./references/api-credentials.md) — [Pruna dashboard](https://dashboard.pruna.ai/) and/or [Replicate API tokens](https://replicate.com/account/api-tokens) |

## Workflow

1. **Classify the next phase** — plan (0), stills (A), audio prep (A2), video (B), assembly (C), final audio (D). See [staged-generation-gate.md](./references/staged-generation-gate.md).
...
Runner flag cheat sheet: [workflow-feedback-gates.md](./references/workflow-feedback-gates.md) (Runner flag reference + per-workflow commands).
...
- [staged-generation-gate.md](./references/staged-generation-gate.md) — phases, templates, partial regen
- [workflow-feedback-gates.md](./references/workflow-feedback-gates.md) — workflow index + CLI flags
- [generation_gate.py](https://github.com/PrunaAI/pruna-skills/tree/main/workflows/_shared/scripts/generation_gate.py) — `generation_status.json` helpers
- Scenario hub: [pruna-generative-pipeline](https://github.com/PrunaAI/pruna-skills/tree/main/plugins/pruna-generative-pipeline/skills/pruna-generative-pipeline/SKILL.md)
