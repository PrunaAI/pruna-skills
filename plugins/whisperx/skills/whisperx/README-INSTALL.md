...
Do **not** use `npx plugins add PrunaAI/pruna-skills@whisperx` — the plugins CLI has no `@name` filter (that’s skills only) and prints “No plugins found”.
...
```bash
npx skills add .@whisperx -y
# or:
npx skills add ./plugins/whisperx/skills --skill whisperx -y
```
