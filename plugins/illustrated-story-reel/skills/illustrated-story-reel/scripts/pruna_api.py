...
def api_seed_from_plan(plan: dict) -> int | None:
    """User-locked API seed only; ritual_seed strings are never passed to Pruna."""
    if plan.get("api_seed") is not None:
        return plan["api_seed"]
    # ponytail: legacy plans used int project_seed as API seed; drop after migration
    legacy = plan.get("project_seed")
