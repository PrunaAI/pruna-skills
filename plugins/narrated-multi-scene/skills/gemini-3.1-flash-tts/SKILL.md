...
**Typical downstream (preferred):** upload MP3/WAV to Pruna `/v1/files` → pass as **`input.audio`** with **`input.image`** and **`input.last_frame_image`** on [`p-video`](../p-video/SKILL.md) ([scene anchor triple](./references/scene-anchor-triple.md)). Same upload pattern on [`p-video-avatar`](https://github.com/PrunaAI/pruna-skills/tree/main/plugins/p-video-avatar/skills/p-video-avatar/SKILL.md) for lip-sync narration.
