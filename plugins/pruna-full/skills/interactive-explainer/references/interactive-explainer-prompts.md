...
6. **`p-video-avatar`:** `defaults.avatar_negative_prompt` is a **Pruna suppression token list** (nouns like `subtitles`, `watermark`) — not creative wording. See [`p_video_avatar_payload.py`](https://github.com/PrunaAI/pruna-skills/tree/main/workflows/_shared/scripts/p_video_avatar_payload.py).
...
Broader blocked-phrase table (keyboards, mirrors, packshots): [visual-variety-bible.md](https://github.com/PrunaAI/pruna-skills/tree/main/shared/visual-variety-bible.md#prompt-patterns).
