...
|----------------|--------|-------------------|-------|
| [interactive-explainer](../../interactive-explainer/SKILL.md) | `workflows/interactive-explainer/scripts/run_from_plan.py` | `stills` | plan → stills → TTS → video → assemble+bed |
| [music-video](../../music-video/SKILL.md) | `workflows/music-video/scripts/run_from_plan.py` | `song` | plan/lyrics → song → align → stills → video → assemble |
| [illustrated-story-reel](../../illustrated-story-reel/SKILL.md) | `workflows/illustrated-story-reel/scripts/run_from_plan.py` | `stills` | plan → stills → tts **or** music → assemble (no p-video) |
| [narrated-multi-scene](../../narrated-multi-scene/SKILL.md) | manual / phased curl | — | plan → stills → TTS → video → bed |
| [visual-transition-reel](../../visual-transition-reel/SKILL.md) | `workflows/visual-transition-reel/scripts/run_from_plan.py` | `stills` | plan → stills → video → assemble+bed |
| [avatar-single-scene](../../avatar-single-scene/SKILL.md) | manual / curl | — | plan → still → avatar |
| [avatar-multi-scene](../SKILL.md) | manual / curl | — | plan → hero+stills → avatar/animate → assembly |
