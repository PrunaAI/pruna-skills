...
**Quality gate:** [p-image-upscale-quality-checklist.md](../../../references/p-image-upscale-quality-checklist.md)

Install: `./scripts/install_skill.sh p-image-upscale-comparison`

### Maintainer recipes (repo clone only — not general options)

Gallery batch scripts live under [`examples/workflows/p-image-upscale-comparison/scripts/`](../../../examples/workflows/p-image-upscale-comparison/scripts/) — do not bundle in portable install.
...
1. **Confirm pair** — Run [p-image-upscale-quality-checklist.md](../../../references/p-image-upscale-quality-checklist.md) on the after still.
2. **Choose regions** — Start from a preset (`portrait`, `product`, `landscape`, `generic`) or define custom normalized rects (`x`, `y`, `w`, `h` in 0–1).
3. **Write config** — Copy [`config.template.json`](../../../examples/workflows/p-image-upscale-comparison/config.template.json) or generate one:
...
```bash
pip install -r examples/workflows/p-image-upscale-comparison/scripts/requirements.txt 2>/dev/null || pip install -r scripts/requirements.txt
export PRUNA_API_KEY="your_key"

python3 examples/workflows/p-image-upscale-comparison/scripts/prepare_restoration_upscale_demo.py
```

Output: `output/p-image-upscale-comparison/restoration-vacation-busy-128mp/`  
...
python3 examples/workflows/p-image-upscale-comparison/scripts/generate_upscale_prompt_examples.py --skip-generate   # missing tiers only
```

Open `output/p-image-upscale-comparison/prompt-examples/gallery.html`.
...
```bash
python3 examples/workflows/p-image-upscale-comparison/scripts/prepare_restoration_upscale_demo.py --skip-generate --skip-degrade
```

Re-render video only:

```bash
python3 examples/workflows/p-image-upscale-comparison/scripts/prepare_restoration_upscale_demo.py --skip-generate --skip-degrade --skip-upscale
```
...
python3 examples/workflows/p-image-upscale-comparison/scripts/prepare_waldo_upscale_demo.py
```

Output directory: `output/p-image-upscale-comparison/nature-pruna-128mp/`  
Re-render only (assets already present):

```bash
python3 examples/workflows/p-image-upscale-comparison/scripts/prepare_waldo_upscale_demo.py --skip-generate --skip-upscale
```
...
- Avatar / motion video (no upscale step): [multi-scene-avatar-video](../multi-scene-avatar-video/SKILL.md)
- Example fixture (optional): [tellers-scene3.fixture.json](../../../examples/workflows/p-image-upscale-comparison/tellers-scene3.fixture.json)
