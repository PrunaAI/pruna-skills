...
Use **[multi-scene-avatar-video](../../../guides/workflows/core/avatar-multi-scene/SKILL.md)** with a scene table mixing **`avatar`** and **`animate`** rows. Model guidance: [animate-beats.md](../../../guides/workflows/core/avatar-multi-scene/animate-beats.md).

## Prompt

> Create a **P-Video-Animate** product announcement (~45–60s). Mix talking-head beats with slider comparison demos.
>
> **Eye-catching variety:** each animate slider row uses 3–4 persona stills in **different visual styles** — photoreal UGC, premium anime, claymation, Disney/Pixar 3D, cyberpunk, blockbuster film, AAA game — each with its **own background, camera angle, and lighting**. Alternate cast gender and representation across avatar rows. See [visual-variety-bible.md](../../../references/shared/visual-variety-bible.md).
>
...
pip install -r guides/workflows/launches/p-video-animate-comparison/scripts/requirements.txt

python3 guides/workflows/launches/p-video-animate-comparison/scripts/run_from_plan.py \
  --plan output/launches/p-video-animate-announcement/announcement_plan.json \
  --out-dir output/launches/p-video-animate-announcement \
  --phase stills
```
...
Config templates: [`examples/workflows/launches/p-video-animate-comparison/`](./)
